...
def fetch_results(event_name):
    """Scrape actual results from UFCStats for a completed event"""
    for status in ['completed']:
        try:
            ...
            for row in soup.select('tr.js-fight-details-click'):
                ...
                fa = links[0].get_text(strip=True)
                fb = links[1].get_text(strip=True)
                # Winner is indicated by 'win' in first col
...
def compute_accuracy(predictions_json, results):
    """Compare model predictions to actual results"""
    correct   = 0
    wrong     = 0
    total     = 0
    by_conf   = {'high': {'c':0,'w':0}, 'medium': {'c':0,'w':0}, 'low': {'c':0,'w':0}}
    ev_hits   = 0
    ev_misses = 0
    fight_results = []

    for fight in predictions_json.get('fights', []):
        if fight.get('not_found'): continue

        fa   = fight['fighter_a']
        fb   = fight['fighter_b']
        pred = fight['winner']
        conf = fight['confidence']

        # Look up actual result
        actual = results.get(f"{fa}|{fb}") or results.get(f"{fb}|{fa}")
        ...
        # EV tracking
        if fight.get('ev_a') and fight.get('ev_a') > 0:
            ev_fighter = fa
            if is_correct and pred == fa:
                ev_hits += 1
            elif pred == fa:
                ev_misses += 1

        if fight.get('ev_b') and fight.get('ev_b') > 0:
            ev_fighter = fb
            if is_correct and pred == fb:
                ev_hits += 1
            elif pred == fb:
                ev_misses += 1
